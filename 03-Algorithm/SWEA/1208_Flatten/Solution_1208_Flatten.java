...
public class Solution_1208_Flatten {
	...
	public static void main(String[] args) throws IOException {
		...
		br = new BufferedReader(new StringReader(src)); // 코드 제출시엔 주석처리
		StringBuilder sb = new StringBuilder();
		for (int t = 1; t <= 10; t++) {
			sb.append("#").append(t).append(" ");
			//////////////////////////////////////// TC 처리

			int dumpCnt = Integer.parseInt(br.readLine()); // 덤프횟수 제한
			int[] boxes = new int[100]; // 100 길이의 상자 높이를 입력 받을 배열 준비
			// StringTokenizer 지정 구분자를 기준으로 문자열을 쪼개주는 역할. 기본으로 공백을 기준으로 쪼갬
			...
			while (true) {
				// 1. 정렬
				Arrays.sort(boxes);
				// 2. 평탄화 됐는지? 덤프 횟수 넘었는지?
				if (boxes[99] - boxes[0] <= 1 || count >= dumpCnt) {

					sb.append(boxes[99] - boxes[0]); // 2-1. 최고점-최저점 값을 저장
					break;
				}
				// 3. 덤프 진행
				boxes[99] -= 1;
