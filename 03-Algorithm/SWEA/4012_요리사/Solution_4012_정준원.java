...
//			for (int i = 0; i < N; i++)
			...
			System.out.println("#" + test_case + " " + res / 2);
	...
	static void tot() {
		...
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < N; j++) {
				if (i != j) {
					if (visit2[i] == true && visit2[j] == true)
						sum += arr[i][j] + arr[j][i];

					if (visit2[i] == false && visit2[j] == false)
						sum2 += arr[i][j] + arr[j][i];
				}
			}
		}

		res = Math.min(res, Math.abs(sum - sum2));
