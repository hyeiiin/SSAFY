...
//		chatBot(N);
	...
	private static void chatBot(int count) {
		...
		for (int i = 0; i <= count; i++) {
			if (i==0) {
				...
				prefixSb.append("____"); //count수만큼 ____ 추가
				String prefix = prefixSb.toString(); //버퍼에 쌓아둔 ____ 만큼 출력
	...
	private static void chatBot2(int count) {
		String str = under;
		if (count == 0) {
			...
			under = under + "____";
			chatBot2(count-1);
			System.out.println(str + "라고 답변하셨지.");			
		}
